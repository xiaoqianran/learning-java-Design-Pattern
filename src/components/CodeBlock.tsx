  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border bg-code-bg shadow-soft">
      ...
      <pre className="scrollbar-thin overflow-x-auto p-4 text-[13px] leading-relaxed">
        <code className={cn("font-mono text-code-fg whitespace-pre")}>{highlightLite(code)}</code>
      </pre>
    </div>
  );
}
...
function colorLine(line: string) {
  ...
  const parts = line.split(
    /(\b(?:import|package|public|private|protected|class|interface|enum|extends|implements|return|new|static|final|void|if|else|for|while|try|catch|throw|throws|abstract|synchronized|volatile|true|false|null)\b|'[^']*'|"[^"]*"|`[^`]*`)/g,
  );
  return parts.map((p, i) => {
    if (!p) return null;
    if (
      /^(import|package|public|private|protected|class|interface|enum|extends|implements|return|new|static|final|void|if|else|for|while|try|catch|throw|throws|abstract|synchronized|volatile|true|false|null)$/.test(
        p,
      )
    ) {
      return (
